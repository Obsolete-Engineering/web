# Obsolete Brand

Shared language for the studio’s identity and the digital experiences that express it.

## Language

**AI product delivery**:
An engagement that turns a specific customer or business problem into a useful AI-enabled product or feature, from validation through launch.
_Avoid_: AI transformation, AI solutions, AI enhancement

**AI-enabled product**:
A customer-facing digital product in which AI enables a specific user outcome rather than serving as a generic label or isolated demonstration.
_Avoid_: AI-powered everything, AI solution, AI demo

**AI fit**:
Evidence that AI materially improves a product outcome enough to justify its added uncertainty and operational cost.
_Avoid_: AI for its own sake, AI mandate

**Production-ready AI product**:
An AI-enabled product fit for its intended real-world use because its quality, failure modes, data boundaries, and operating responsibilities are understood.
_Avoid_: Demo dressed as a product, prototype in production

**AI product improvement**:
Ongoing work informed by real use to improve an AI-enabled product’s value, behavior, and safeguards after launch.
_Avoid_: Model tuning, maintenance-only support

**AI product judgment loop**:
A delivery cycle that establishes AI fit, tests the product with real users and representative data, prepares it for dependable use, and learns from production evidence.
_Avoid_: AI implementation pipeline, one-off prototype

**Applied AI product engineering**:
The discipline of turning existing AI capabilities into dependable product behavior, rather than researching or training foundation models.
_Avoid_: Foundation-model research, model lab

**Creative product team**:
A team that owns a customer-facing digital product and values distinctiveness as much as utility.
_Avoid_: Any business, companies exploring AI

**Contact sculpture**:
An abstract field of dense, ink-black contour strands folded around a large left-facing void that remains open to the orange contact field and returns attention toward the invitation. Its camera, orientation, and overall silhouette remain fixed; at rest only minute strand tension shifts create a near-still “breath.” Pointer proximity creates a shallow local dent, while a click deepens the press and releases one controlled ripple with elastic lag. Rapid clicks merge into the active ripple and redirect its origin within a capped energy budget rather than stacking waves. Smooth response takes priority over strand count or render resolution; a static mesh poster preserves its place when live 3D is unavailable, reduced motion is requested, or adaptive quality cannot sustain fluid motion.
_Avoid_: Contact mascot, idea visualizer, particle-cloud reprise, woven grid, triangulated wireframe, sphere, torus, camera parallax, rotating or drag-to-spin object, sound effects

**Contact stage**:
The upper contact composition, enabled only at 1024px or wider with a fine hover pointer, in which the invitation headline and Contact sculpture act as equal visual leads with roughly a 55/45 split and a controlled crop at the sculpture’s right edge. Fit guidance and the inquiry row span beneath both; smaller screens retain the existing text-led stacked composition without the sculpture.
_Avoid_: Full-width desktop headline with side decoration, sculpture-dominated contact section, mobile sculpture, text overlap, interaction instructions, custom contact cursor

**Contact tension reveal**:
The once-per-page-visit contact-stage entrance in which a loose compressed mesh pulls taut into the folded void as the orange takeover and invitation copy settle together. Pointer interaction remains quiet until the resolved silhouette has settled, then becomes available immediately; later scroll returns show the resolved interactive form.
_Avoid_: Spinning product-render entrance, independent draw-on effect, unrelated motion montage

**Conversion-safe spectacle**:
A memorable contact payoff that invites brief exploration without delaying, obscuring, or visually demoting the project-inquiry message and action.
_Avoid_: Decorative afterthought, interaction-first toy, technical showcase

**Interactive process field**:
A warm-paper, ink-line diagram that connects AI product outcomes to the AI product judgment loop through restrained pointer and focus responses, using orange only for active connections and remaining understandable without interaction.
_Avoid_: Fake chat interface, neural-network motif, decorative node map

**Integrated hero surface**:
A hero whose full-bleed symbol field sits behind the existing centred composition and masthead. The visual surface and content are choreographed as one experience while preserving the composition’s hierarchy and final geometry.
_Avoid_: Separate shader panel, animation above the copy, recomposed hero content

**Hero ceremony**:
A brief first-view sequence in which the symbol field leads and the masthead, typography, clarification, and actions are progressively revealed into their established composition.
_Avoid_: Splash screen, loader theatre, hero redesign

**Ceremony fail-open**:
The immediate resolved hero shown when the live field is not ready or cannot sustain smooth ceremony motion, preserving comprehension and interaction instead of waiting or imitating the sequence.
_Avoid_: Loader, indefinite hold, weakened poster-only ceremony

**Field awakening**:
The governing idea for the hero ceremony: the symbol field begins latent, receives one restrained orange impulse, and gathers into its living resting state as the interface resolves from the same energy.
_Avoid_: Aperture wipe, generic zoom reveal, typography-driven spectacle

**Punctuation impulse**:
The single orange disturbance that originates at the future position of the headline’s period; orange remains local while a broader graphite pressure response awakens the field and connects it to the typographic reveal.
_Avoid_: Random splat, orange edge-to-edge ring, decorative flare

**Ceremonial causality**:
The primary quality measure for the hero ceremony: every field, identity, typographic, and interaction beat feels like a consequence of the punctuation impulse rather than a stack of independent effects.
_Avoid_: Maximum intensity, effect montage, ornamental timing

**Hero imagery**:
The symbol field itself as the hero’s complete visual image, shaped through motion, depth, colour, and light without a competing photographic or project-image layer.
_Avoid_: Project glimpse, background photography, persistent image overlay

**Warm paper symbol field**:
A dense edge-to-edge field of tiny graphite checker, cross, and four-point marks on warm paper. Broad low-contrast density bands and gentle displacement keep it atmospheric without competing with the headline.
_Avoid_: Blank paper, sparse particles, branded icons, coarse pixels, neon field

**Fluid trail**:
A connected velocity-driven wake that bends nearby symbols and temporarily turns them orange along an eased pointer path, then dissipates without a visible reset.
_Avoid_: Full-height pointer wipe, persistent paint stroke, cursor spotlight, solid high-contrast ribbon

**Tap pulse**:
The simplified touch equivalent of fluid displacement: one subtle local disturbance from a stationary tap while native swipe scrolling remains uninterrupted.
_Avoid_: Touch painting, scroll-blocking drag

**Hero composition**:
The existing centred hierarchy of eyebrow, proposition, clarification, and actions. The visual surface changes around it; the message order does not.
_Avoid_: Split hero, shader showcase with secondary copy

**Quiet ceremony response**:
The field’s restrained graphite-only displacement in response to pointer or tap input during the hero ceremony, before the established orange fluid trail becomes available in the explorable hero.
_Avoid_: Input lockout, competing orange trails, visitor-steered sequence

**Explorable hero**:
The settled hero state in which the ceremony’s last wake dissipates continuously into the established ambient field and the normal fluid trail and tap pulse are available without a mode-change cue.
_Avoid_: Simulation reset, persistent ceremony residue, interaction tutorial

**Hero pointer dot**:
A small orange follower shown only over non-interactive hero space while the native cursor remains unchanged. It is hidden over controls, on touch input, and for reduced motion.
_Avoid_: Custom cursor replacement, full-page cursor follower, glowing cursor

**Stable masthead**:
A transparent masthead sharing the warm paper field whose identity and navigation settle crisp, ink-coloured, and outside the shader’s distortion after a brief visual reveal.
_Avoid_: Separate header strip, shader-distorted navigation

**Stable hero content**:
The headline, eyebrow, clarification, and actions preserve their semantic order, final geometry, colour, and interaction behavior while allowing a brief visual reveal during the hero ceremony.
_Avoid_: Reactive copy, colour-shifting controls, rearranged hero content

**Crisp line resolve**:
The typographic handoff from field to interface: the eyebrow and headline settle line by line without entering the shader, the orange period locks the transition, and supporting copy and actions follow.
_Avoid_: Letter particles, text distortion, density silhouette

**Shallow material depth**:
A temporary sense of depth created as the softened, slightly enlarged symbol field responds locally to the punctuation impulse and settles into its established flat surface.
_Avoid_: Camera flight, dramatic zoom, pointer-tilted plane

**Tonal material light**:
The impression of illumination created through symbol sharpness, density, and restrained paper-to-graphite contrast rather than a visible digital light source.
_Avoid_: Glow, bloom, radial spotlight

**Precise fluid motion**:
Slow, incommensurate ambient waves combine with slight elastic pointer follow and a short local dissolve. Motion remains continuous, low-amplitude, and free of obvious loop resets.
_Avoid_: Raw cursor stamping, exaggerated lag, frantic splatter, fixed digital brush

**Fine geometric symbols**:
Small deterministic checker, cross, and four-point modules that read as a contemporary monochrome texture at normal viewing distance rather than overt pixel art.
_Avoid_: Coarse Bayer blocks, retro-computer texture, recognizable logos
