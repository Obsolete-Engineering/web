...
const loopFrame = (frame: number, duration: number) => ((frame % duration) + duration) % duration;
...
const OldSystemFragment = ({
  ...
  const driftIn = interpolate(
    frame,
    [0, 70, 150, 215, 240, 310, 360],
    [-72 + delay * 0.7, 0, 18, 330, 330, -72 + delay * 0.7, -72 + delay * 0.7],
    {
      ...CLAMP,
      easing: easeInOut,
    },
  ...
  const opacity = interpolate(
    frame,
    [0, 58, 150, 225, 270, 310, 360],
    [0.34, 0.96, 1, 0.48, 0.03, 0.02, 0.34],
    CLAMP,
  );
  const scale = interpolate(
    frame,
    [150 + delay * 0.2, 230 + delay * 0.2, 310, 360],
    [1, 0.38, 0.8, 1],
    CLAMP,
  );
  const rotation = interpolate(
    frame,
    [0, 125, 225, 360],
    [disorder, disorder * 0.52, 0, disorder],
    CLAMP,
  );
  const highlight =
    Math.max(0, 1 - Math.abs(scanX - (x + driftIn + 65)) / 86) *
    interpolate(frame, [70, 90, 155, 180], [0, 1, 1, 0], CLAMP);
  ...
  return (
    <g
      transform={`translate(${x + driftIn} ${y + pullY + drift(frame, 5, delay)}) rotate(${rotation}) scale(${scale})`}
      opacity={opacity}
    >
      <FloatingPanel
        x={0}
        ...
        slant={18}
        fill={colors.panel}
        stroke={highlight > 0.12 ? colors.signal : colors.ink}
        opacity={1}
        depth={8}
      >
        <rect
...
const ChartMess = () => (
  <g>
    <path
      d="M22 68 C40 34 50 78 67 46 S91 30 122 58"
      fill="none"
      stroke={colors.ink}
      strokeWidth={1.4}
      opacity={0.76}
    />
    <path
      d="M22 58 C40 72 47 38 70 58 S94 70 126 34"
      fill="none"
      stroke={colors.signal}
      strokeWidth={1.1}
      opacity={0.44}
...
const WorkflowMess = () => (
  ...
  >
    <path
      d="M24 54 C42 22 69 79 88 38 S112 46 124 25"
      stroke={colors.ink}
      strokeWidth={1.3}
      opacity={0.75}
    />
    <path
      d="M28 30 C47 72 75 20 112 66"
      stroke={colors.signal}
      strokeWidth={1.1}
      opacity={0.42}
    />
...
const InterfaceLayer = ({
  ...
  const layerX =
    x + interpolate(frame, [70, 150, 240, 360], [0, index % 2 === 0 ? -10 : 10, 0, 0], CLAMP);
...
const LogicLayerContent = ({ frame }: { frame: number }) => {
  ...
  return (
    <g opacity="0.86">
      <path
        d="M54 34 H122 V56 H185 V38 H250"
        fill="none"
        stroke={colors.ink}
        strokeWidth="1.3"
        strokeDasharray="72"
      ...
      />
      <path
        d="M58 101 H110 V78 H170 V104 H258"
        fill="none"
        stroke={colors.softLine}
        strokeWidth="1.2"
        strokeDasharray="7 8"
      ...
      {[54, 122, 185, 250, 110, 170].map((cx, index) => (
        <circle
          key={`logic-node-${cx}-${index}`}
          cx={cx}
          cy={index < 4 ? (index === 2 ? 56 : 34 + (index % 2) * 22) : index === 4 ? 78 : 104}
          r="7"
          fill={colors.panel}
          stroke={index === 5 ? colors.green : colors.softLine}
        />
      ))}
      <rect
        x="30"
...
const ReplacementEngine = ({ frame }: { frame: number }) => {
  ...
  const settlePulse = spring({
    frame: frame - 226,
    fps,
    config: { damping: 28, mass: 0.55, stiffness: 120 },
  });
      ...
      >
        ...
        {[0, 1, 2].map((item) => (
          <rect
            ...
            height="12"
            rx="3"
            fill={item === 1 ? colors.signal : colors.line}
            opacity={item === 1 ? 0.35 : 0.8}
          />
        ))}
      </g>
    </g>
...
const CleanSystemPanel = ({
  ...
  return (
    <g
      transform={`translate(${x + slide} ${y + snap + drift(frame, 2.4, delay * 9)})`}
      opacity={opacity}
    >
      <FloatingPanel
        width={kind === 'nav' ? 84 : 210}
        height={kind === 'table' ? 112 : 92}
        x={0}
        y={0}
        slant={18}
        fill={colors.panel}
  ...
  <g>
    ...
    {[40, 55, 70].map((y, index) => (
      <rect
        ...
        height="7"
        rx="3.5"
        fill={index === 1 ? colors.green : colors.line}
        opacity={index === 1 ? 0.26 : 0.82}
      />
    ))}
  </g>
);
...
export const ObsoleteHero = ({ showLabels = true, reducedMotion = false }: ObsoleteHeroProps) => {
  ...
  const orangeStatusOpacity = interpolate(
    frame,
    [0, 30, 145, 210, 360],
    [0.25, 1, 1, 0.25, 0.25],
    CLAMP,
    ...
    <AbsoluteFill style={{ backgroundColor: colors.paper }}>
      ...
      >
        ...
        <g transform="translate(600 450) scale(1.16) translate(-600 -450)">
          ...
          >
            <path
              d="M285 728 C420 785 714 790 909 717"
              strokeDasharray="5 12"
            />
            <path
              d="M339 192 C485 118 726 116 864 186"
