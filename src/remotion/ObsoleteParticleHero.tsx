...
const addParticle = (
  ...
  particles.push({
    ...
    size: options.size ?? (isSignal ? 3.1 + random() * 1.35 : isEdge ? 2.1 + random() * 1.05 : 1.45 + random() * 0.95),
    depth: projectedHome.z,
    delay: options.delay ?? clamp((1 - edgeBias) * 0.28 + random() * 0.28),
    phase: options.phase ?? random() * TAU,
    color: options.color ?? (isSignal ? colors.signal : isEdge ? colors.ink : random() < 0.28 ? colors.ink : colors.muted),
    opacity: options.opacity ?? (isSignal ? 0.84 : isEdge ? 0.78 : isDimFace ? 0.32 + random() * 0.18 : 0.5 + random() * 0.28),
    homeZ: z,
    pyramidZ: z,
    logoZ: z,
    disperseZ,
...
const addCubeEdges = (particles: Particle[], random: () => number) => {
  ...
  corners.forEach((y) => {
    corners.forEach((z) => {
      addLine3D(particles, random, [-1, y, z], [1, y, z], 54, "edge", {delay: 0.02});
    });
  });

  corners.forEach((x) => {
    corners.forEach((z) => {
      addLine3D(particles, random, [x, -1, z], [x, 1, z], 54, "edge", {delay: 0.02});
    });
  });

  corners.forEach((x) => {
    corners.forEach((y) => {
      addLine3D(particles, random, [x, y, -1], [x, y, 1], 54, "edge", {delay: 0.02});
    });
  });
};

const addInterfaceMarks = (particles: Particle[], random: () => number) => {
  [-0.48, -0.18, 0.12].forEach((y, index) => {
    addLine3D(
      particles,
      random,
      [-0.64 + index * 0.04, y, 1.012],
      [0.34 - index * 0.12, y, 1.012],
      28,
      "front",
      {color: colors.ink, opacity: 0.74, size: 2.15, delay: 0.14 + index * 0.04},
    );
    addLine3D(
      particles,
      random,
      [-0.64 + index * 0.04, y - 0.1, 1.012],
      [0.06 - index * 0.08, y - 0.1, 1.012],
      18,
      "front",
      {color: colors.softLine, opacity: 0.48, size: 1.55, delay: 0.2 + index * 0.05},
    );
  });
  ...
  addLine3D(
    ...
    18,
    "signal",
    {color: colors.signal, opacity: 0.9, size: 3.05, delay: 0.04},
  );
};
...
const createParticles = () => {
  ...
  addInterfaceMarks(particles, random);
...
const getParticleFrameState = (
  ...
  const {pyramid, logo, cube, explode} = reducedMotion
    ? {pyramid: 0, logo: 0, cube: 0, explode: 0}
  ...
  const afterPyramidBurstDepth = lerp(particle.disperseDepth, particle.pyramidDisperseDepth, pyramid);
  ...
  const waveY = Math.cos(t * TAU + particle.phase + baseX * 0.009) * waveAmount * 0.72 * particle.mobility;
  const swayX = Math.sin(t * Math.PI + particle.phase * 0.8 + baseY * 0.004) * sway * 8 * particle.mobility;
  const swayY = Math.cos(t * Math.PI + particle.phase * 0.7 + baseX * 0.004) * sway * 6 * particle.mobility;
  const curveX = Math.sin(easedExplode * Math.PI + particle.phase) * 28 * explode * particle.mobility;
  const curveY = -Math.cos(easedExplode * Math.PI + particle.phase * 0.7) * 18 * explode * particle.mobility;
  const firstSignal =
  ...
  const signalPeak = particle.face === "signal" ? Math.max(firstSignal, secondSignal, thirdSignal) : 0;
  const x =
    homeX +
    idleFloatX +
    waveX +
