...
const addParticle = (
	...
	const mobility =
		options.mobility ?? clamp(0.38 + edgeBias * 0.46 + random() * 0.26);
	const distance =
		face === "signal" ? 1.42 : 0.82 + mobility * 1.28 + random() * 0.62;
...
const addFaceParticles = (
	...
	const density =
		face === "back" || face === "left" || face === "bottom" ? 0.42 : 0.72;
...
const assignPyramidTargets = (particles: Particle[]) => {
	...
	particles.forEach((particle, index) => {
		...
		const projectedPyramid = project(
			pyramidTarget.x,
			pyramidTarget.y,
			pyramidTarget.z,
		);
		...
		const distance =
			particle.face === "signal"
				? 1.35
				: 0.86 + particle.mobility * 1.34 + random() * 0.5;
		...
		const projectedDisperse = project(
			pyramidDisperse.x,
			pyramidDisperse.y,
			pyramidDisperse.z,
		);
...
const LOGO_FACE_CENTER_X =
	((logoBounds.minX + logoBounds.maxX) / 2) * LOGO_SCALE_X;
const LOGO_FACE_CENTER_Y =
	((logoBounds.minY + logoBounds.maxY) / 2 + LOGO_Y_OFFSET) * LOGO_SCALE_Y;
...
const sampleLogoFill = (index: number, random: () => number): Point2D => {
	for (let attempt = 0; attempt < 360; attempt++) {
		const u =
			(index * 0.754877666 + attempt * 0.381966011 + random() * 0.024) % 1;
		const v =
			(index * 0.569840296 + attempt * 0.618033989 + random() * 0.024) % 1;
		const point = {
...
const createLogoTarget = (
	particle: Particle,
	index: number,
	random: () => number,
): LogoTarget => {
	const traceEdge =
		particle.face === "edge" || particle.face === "signal" || random() < 0.1;
	const logoPoint = traceEdge
		? sampleLogoBoundary(index, random)
		: sampleLogoFill(index, random);
	const backLayer =
		(particle.face === "back" ||
			particle.face === "left" ||
			particle.face === "bottom") &&
		random() < 0.72;
...
const assignLogoTargets = (particles: Particle[]) => {
	...
	particles.forEach((particle, index) => {
		...
		const distance =
			particle.face === "signal"
				? 1.34
				: 0.82 + particle.mobility * 1.26 + random() * 0.48;
		const logoDisperse = {
...
const createParticles = () => {
	...
	(["front", "right", "top", "back", "left", "bottom"] as const).forEach(
		(face) => {
...
const getMorphState = (t: number, delay: number) => {
	const pyramid = smoothstep(
		delayedFrame(124, delay, 16),
		delayedFrame(226, delay, 10),
		t,
	);
	const logo = smoothstep(
		delayedFrame(418, delay, 16),
		delayedFrame(528, delay, 12),
		t,
	);
	const cube = smoothstep(
		delayedFrame(724, delay, 18),
		delayedFrame(850, delay, 14),
		t,
	);
...
const getParticleFrameState = (
	...
	const afterPyramidDepth = lerp(
		particle.depth,
		particle.pyramidDepth,
		pyramid,
	);
	...
	const afterPyramidBurstX = lerp(
		particle.disperseX,
		particle.pyramidDisperseX,
		pyramid,
	);
	const afterPyramidBurstY = lerp(
		particle.disperseY,
		particle.pyramidDisperseY,
		pyramid,
	);
	...
	const afterLogoBurstX = lerp(
		afterPyramidBurstX,
		particle.logoDisperseX,
		logo,
	);
	const afterLogoBurstY = lerp(
		afterPyramidBurstY,
		particle.logoDisperseY,
		logo,
	);
	const afterLogoBurstDepth = lerp(
		afterPyramidBurstDepth,
		particle.logoDisperseDepth,
		logo,
	);
	...
	const waveAmount = reducedMotion
		? 0
		: (5.8 + sway * 5.4) * (1 - logoWeight * 0.38);
	const depthBreath = reducedMotion ? 0 : loop01 * 0.018;
	const idleScale =
		(particle.face === "signal" ? 10 : particle.face === "edge" ? 6.4 : 8.2) *
		logoStillness;
	...
	const waveX =
		Math.sin(t * TAU + particle.phase + baseY * 0.012) *
		waveAmount *
		particle.mobility;
	const waveY =
		Math.cos(t * TAU + particle.phase + baseX * 0.009) *
		waveAmount *
		0.72 *
		particle.mobility;
	const swayX =
		Math.sin(t * Math.PI + particle.phase * 0.8 + baseY * 0.004) *
		sway *
		8 *
		particle.mobility;
	const swayY =
		Math.cos(t * Math.PI + particle.phase * 0.7 + baseX * 0.004) *
		sway *
		6 *
		particle.mobility;
	const curveX =
		Math.sin(easedExplode * Math.PI + particle.phase) *
		28 *
		explode *
		particle.mobility;
	const curveY =
		-Math.cos(easedExplode * Math.PI + particle.phase * 0.7) *
		18 *
		explode *
		particle.mobility;
	const firstSignal =
	...
	const signalPeak =
		particle.face === "signal"
			? Math.max(firstSignal, secondSignal, thirdSignal)
			: 0;
	const x =
		homeX +
		idleFloatX +
	...
	const logoAlpha =
		particle.face === "edge" ? 0.92 : 0.66 + particle.logoTone * 0.25;
	const alpha = clamp(
		lerp(movingAlpha, logoAlpha * (1 - easedExplode * 0.2), logoWeight),
	);
	...
	return {
		...
		renderSize:
			...
			(1 +
				signalPeak * 0.42 +
				loop01 * 0.04 +
				logoWeight * (0.18 + particle.logoTone * 0.24)),
		renderColor,
	};
};
...
const ParticleCanvas = ({ reducedMotion }: { reducedMotion: boolean }) => {
	...
	useLayoutEffect(() => {
		...
		drawScene(
			context,
			particles,
			reducedMotion ? 0 : frame % DURATION,
			reducedMotion,
		);
...
export const ObsoleteParticleHero = ({
	reducedMotion = false,
}: ObsoleteParticleHeroProps) => {
